...
//        thetaVariance = ofMap(dna.genes[5],0,1, 0, 30);
        ...
        leafColorA = ofMap(dna.genes[14],0,1,0,25);
        leafShape = ofMap(dna.genes[15],0,1,1,5);
...
void Tree::branch(float b_length, float theta, float b_width)
{
    ...
    if(b_length > minLength )
    {
        ...
        ofRotate(theta + (ofNoise(thetaVariance, ofGetFrameNum()/60.0)*100));
